...
struct VehicleStruct        // 车辆 读取配置文件结构体 要与配置文件中的一致
{
    int id_ = 0;              // 序号
...
public:
    ...
    // vehicle  读取车辆配置参数 成员函数，用来访问private中实例的变量
    void read_vehicle_config(VehicleStruct &vehicle, const std::string &name);
    void read_vehicles_config();
    // 内联函数 函数实现要简短  内联函数可以把整个函数当做一个宏定义，可以加快程序的运行速度
    inline VehicleStruct main_car() const { return main_car_; }     // 通过内联函数在编译的过程中，就return了private中的一些struct实例变量，只能获取不能修改
    ...
    inline std::unordered_map<int, VehicleStruct> obs_pair() const { return obs_pair_; }    // 获取哈希表键值对
...
private:
    ...
    std::unordered_map<int, VehicleStruct> obs_pair_;       // 创建哈希表对应关系
    // 其他模块的实例化
    // pnc_map
